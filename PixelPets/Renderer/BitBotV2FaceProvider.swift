...
struct BitBotFaceProvider: FaceProvider {
    ...
    func draw(in ctx: GraphicsContext, state: PetState, frame: Int, scale: CGFloat) {
        ...
        case .thinking:
            drawThinking(ctx, frame: frame, scale: scale)
        case .typing:
            drawTyping(ctx, frame: frame, scale: scale)
        case .success:
            drawSuccess(ctx, frame: frame, scale: scale)
        case .error:
            drawError(ctx, frame: frame, scale: scale)
        case .sleeping:
            drawSleeping(ctx, frame: frame, scale: scale)
        case .auth:
            drawAuth(ctx, frame: frame, scale: scale)
        default:
            drawIdle(ctx, frame: frame, scale: scale)
        }
    }
    ...
    private func drawThinking(_ ctx: GraphicsContext, frame: Int, scale: CGFloat) {
        ...
        for x in 0..<8 {
            ctx.fillPixel(
                x: x + 4,
                y: 7,
                color: Self.thinkingColor.opacity(x == scan ? 1.0 : 0.3),
                scale: scale
            )
        }
    }
    ...
    private func drawSuccess(_ ctx: GraphicsContext, frame: Int, scale: CGFloat) {
        for (x, y) in [(5, 6), (9, 6), (4, 7), (10, 7)] {
            ctx.fillPixel(x: x, y: y, color: Self.eyeColor, scale: scale)
        }
        for x in 5..<10 {
            ctx.fillPixel(x: x, y: 9, color: Self.eyeColor, scale: scale)
        }
    }

    private func drawError(_ ctx: GraphicsContext, frame: Int, scale: CGFloat) {
        let color = (frame % 10) < 5 ? Self.errorRed : Self.eyeColor
    ...
    private func drawSleeping(_ ctx: GraphicsContext, frame: Int, scale: CGFloat) {
        for (x, y) in [(4, 8), (5, 8), (8, 8), (9, 8)] {
            ctx.fillPixel(x: x, y: y, color: Self.sleepGray, scale: scale)
        }
    }

    private func drawAuth(_ ctx: GraphicsContext, frame: Int, scale: CGFloat) {
        let pulse = 0.6 + 0.4 * abs(sin(Double(frame) * 0.1))
        let color = Self.authYellow.opacity(pulse)
        for (x, y) in [(6, 5), (7, 5), (5, 6), (8, 6)] {
            ctx.fillPixel(x: x, y: y, color: color, scale: scale)
        }
        for y in 7..<10 {
            for x in 5..<9 {
